...
loop { //creates a loop for the bottom code and closed it off with } at end. Allows user to input multiple times until guess matches with secret #
    ...
    io::stdin()
        .read_line(&mut guess)//& means its a reference. A refernce gives us a way to let mutplie parts of code access one piece of data. Ref (&) are immutable so we have to write &mut to make the ref 'guess' mutable 
        .expect("Failed to read line"); //We write a .expect as a backup for errors. If errors occur it will read "failed to read line"
    
    let guess: u32 =  match guess.trim().parse() { //trim: eliminates white space, parse: converts a string to another type, hence we convert from string to number,expect: If error occurs, say if input is not a #, we output the message "please type a number"
        Ok(num) => num, //We eliminated .expect anmd added "match". In this case: when pasrse returns a result we get a Ok and Err, we use match to match those outputs. If its a # then we say its ok or pass, if it's not, we say Err to continue program. We use '_' as a catch all value.
        Err(_) => continue,
    };
...
/* This project was an introduction to Rust concepts such as:
    ...
    Self-Reflection: Gosh this was a little hard to memorize nad comprehend, but reading the notes in the rust doc was helpful. The concept is easy to memorize once you know how it works. 
